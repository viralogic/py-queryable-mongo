...
class TestDecompilers(TestCase):
    ...
    def test_simple_lambda(self):
        ...
        tree = decompiler.decompile(
            (lambda x: x.name == "Western Hockey League").__code__
        )
        self.assertEqual(
    ...
    def test_and_lambda(self):
        ...
        tree = decompiler.decompile(
            (
                lambda x: x.name == "Western Hockey League"
                and x.short_name == "WHL"
            ).__code__
        )
        self.assertEqual(
    ...
    def test_dict(self):
        decompiler = LambdaDecompiler()
        tree = decompiler.decompile(
            (lambda x: {"name": x.name, "season": x.season}).__code__
        )
        self.assertEqual(
    ...
    def test_in(self):
        decompiler = LambdaDecompiler()
        tree = decompiler.decompile(
            (lambda x: x.name in ["Western Hockey League"]).__code__
        )
        self.assertEqual(
    ...
    def test_div(self):
        decompiler = LambdaDecompiler()
        tree = decompiler.decompile(
            (lambda x: x.goals_against / x.games_played).__code__
        )
        self.assertEqual(
    ...
    def test_boolop_brackets_or(self):
        decompiler = LambdaDecompiler()
        tree = decompiler.decompile(
            (
                lambda x: (x.gpa >= 10 and x.gpa <= 50)
                or x.last_name == "Fenske"
            ).__code__
        )
        self.assertEqual(
            "Lambda(args=arguments(args=[Name(id='x', ctx=Param())], vararg=None, kwarg=None, defaults=[]), body=Return(value=BoolOp(op=Or(), values=[BoolOp(op=And(), values=[Compare(left=Attribute(value=Name(id='x', ctx=Load()), attr='gpa', ctx=Load()), ops=[GtE()], comparators=[Num(n=10)]), Compare(left=Attribute(value=Name(id='x', ctx=Load()), attr='gpa', ctx=Load()), ops=[LtE()], comparators=[Num(n=50)])]), Compare(left=Attribute(value=Name(id='x', ctx=Load()), attr='last_name', ctx=Load()), ops=[Eq()], comparators=[Str(s='Fenske')])])))",
    ...
    def test_boolop_brackets_and(self):
        decompiler = LambdaDecompiler()
        tree = decompiler.decompile(
            (
                lambda x: (x.gpa >= 10 or x.gpa <= 50)
                and x.last_name == "Fenske"
            ).__code__
        )
        self.assertEqual(
            "Lambda(args=arguments(args=[Name(id='x', ctx=Param())], vararg=None, kwarg=None, defaults=[]), body=Return(value=BoolOp(op=And(), values=[BoolOp(op=Or(), values=[Compare(left=Attribute(value=Name(id='x', ctx=Load()), attr='gpa', ctx=Load()), ops=[GtE()], comparators=[Num(n=10)]), Compare(left=Attribute(value=Name(id='x', ctx=Load()), attr='gpa', ctx=Load()), ops=[LtE()], comparators=[Num(n=50)])]), Compare(left=Attribute(value=Name(id='x', ctx=Load()), attr='last_name', ctx=Load()), ops=[Eq()], comparators=[Str(s='Fenske')])])))",
            ast.dump(tree),
        )
